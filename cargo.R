...
for (i in 1:length(weights)) 
{
  for (j in 0:capacity) 
  {
    ...
    {
      dp[i+1, j+1] <- max(dp[i, j+1], dp[i+1, j+1-weights[i]] + revenue[i])
    }
  }
}
...
{
  while (j >= weights[i] && dp[i+1, j+1] == dp[i+1, j+1-weights[i]] + revenue[i]) 
  {
...
cat("Maximum revenue of loaded cargos:", dp[length(weights)+1, capacity+1], "\n")
